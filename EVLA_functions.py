...
def get_data_arrays(msfile, *args):
    """This function returns a list of data arrays that have been selected in 
       the *args argument. *args should be formatted as a list of lists in 
       which each data type is the first entry, and the second is establishes
       if we need to use a selection criterion."""

    N = len(args[0]) # the number of data types we need to retrieve
    final = [] # A list for all the data arrays
    tb.open(msfile)
    for i in range(0, N): # loop through each of the data types
        ### Step I: Create an instance of the table object for the current
        ###         data type.
        TAQLcmd = 'select '+args[0][i][0]+' from '+msfile
        if args[0][i][1] == 'uvdist': 
            if args[0][i][0] != 'CHAN_FREQ':
                TAQLcmd += ' where sqrt(sumsqr(UVW[:2])) < 1600 && \
sqrt(sumsqr(UVW[:2])) > 1500 '
            else:
                TAQLcmd += '/SPECTRAL_WINDOW '
        if args[0][i][1] == 'none': 
            if args[0][i][0] == 'CHAN_FREQ':
                TAQLcmd += '/SPECTRAL_WINDOW '
        if args[0][i][1] == 'sw': 
            TAQLcmd += ' where DATA_DESC_ID == '+args[0][i][2]+' '
                
        temp_table = tb.taql(TAQLcmd) # creates an object with data table

        ### Step II. Turn the data into a numpy array
        nr = temp_table.nrows() # calculates the number of rows
        if args[0][i][2] == '':
            if args[0][i][0] == 'DATA' or args[0][i][0] == 'FLAG':
                temp_array = np.zeros(nr*2048*2).reshape(2,2048,nr)
        else:
            temp_array = np.zeros(nr)
        temp_array = temp_table.getcol(args[0][i][0])
...
def target_time_avg(msfile):
    """return the relevant data for the target amplitude vs channel plot."""
    
    final = []
    i = 0
    tb.open(msfile)
    while i < 26: # loop through each antenna
        j = i + 1
        while j < 27: # loop through the entire pair
            ### Get the Data.
            TAQLcmd1 = 'select DATA from '+msfile+' where ANTENNA1'\
                      +' == '+str(i)+' && ANTENNA2 == '+str(j)
            datatb = tb.taql(TAQLcmd1)
            TAQLcmd2 = 'select FLAG from '+msfile+' where ANTENNA1'\
                      +' == '+str(i)+' && ANTENNA2 == '+str(j)
            flagtb = tb.taql(TAQLcmd2)
            ### Put the data into arrays
            nr = datatb.nrows() # calculates the number of rows
            tempd = np.zeros(nr*2048*2).reshape(2,2048,nr)
            tempd = datatb.getcol('DATA')
            tempf = np.zeros(nr*2048*2).reshape(2,2048,nr)
            tempf = flagtb.getcol('FLAG')
            duse = flag_mask(tempd, tempf, True)
            final.append(np.nanmean(duse, axis = 2))
            j += 1
        i += 1
    tb.close()
    return final
...
def flag_mask(datav, flagv, flg_param):
    """Mask either the flagged data, or the unflagged data, depending on what 
       the user is trying to flag."""

    if flg_param == True: # this returns the unflagged data
        flagv1 = (flagv-1)*-1 # reverse the boolean condition
        result = datav*flagv1 # mask the data
        result[result==0]=complex(np.nan,np.nan)# turn any masked data into nan
        return result
    if flg_param == False: #get the flagged data
        result = datav*flagv # mask the data
        result[result==0]=complex(np.nan,np.nan)# turn any masked data into nan
        return result


def mask_check_plot(visib):
    """Return the plotting axis for the uvspectrum of a source."""

    argument=[['DATA','uvdist',''],['FLAG','uvdist',''],['CHAN_FREQ','uvdist','']] 
    all_elements=get_data_arrays(visib, argument) # data arrays

    uflag=flag_mask(all_elements[0], all_elements[1], True) # get unflagged data
    flag=flag_mask(all_elements[0], all_elements[1], False) # get flagged data
    
    amps=[np.absolute(uflag),np.absolute(flag)] # calculates the amplitude

    freq=all_elements[2].squeeze()/(10**9)
    xaxis1 = np.array([freq,]*len(amps[0][0][0]))
    xaxis = np.swapaxes(xaxis1,0,1)

    return [xaxis, amps]


def amp_phase_time(visib, specw):
    """Create the axis amplitude and phase plots."""

    argument=[['DATA','sw',specw],['FLAG','sw',specw],['TIME','sw',specw]]
    all_elements = get_data_arrays(visib, argument)

    uflag=flag_mask(all_elements[0], all_elements[1], True) # get unflagged data

    amp = np.absolute(uflag)
    phase = np.angle(uflag,deg=True)

    return [all_elements[2].squeeze(),amp,phase]
    

def uvspec(visib):
    """Plot the uvspectrum."""

    argument=[['DATA','none',''],['FLAG','none',''],['CHAN_FREQ','none','']] 
    all_elements=get_data_arrays(visib, argument) # data arrays

    uflag=flag_mask(all_elements[0], all_elements[1], True) # get unflagged data
    base_time_avg = np.nanmean(uflag,axis=2)
    amp = np.absolute(base_time_avg)
    
    xaxis=all_elements[2].squeeze()/(10**9)

    return [xaxis,amp]


def target_plots_v2(visil, sw):
    """Get the data for the plots and return compact arrays."""

    specw = str(sw)

    ### Get some relevant numbers from the visibilities
    tb.open(visil)
    taqlcmd = 'select SCAN_NUMBER from '+visil
    scans_tb = tb.taql(taqlcmd)
    scans_unsort = scans_tb.getcol('SCAN_NUMBER')
    scans_full = np.unique(scans_unsort, return_counts = True)
    scans = scans_full[0]
    
    taqlcmd = 'select CHAN_FREQ from '+visil+'/SPECTRAL_WINDOW'
    chans_tb = tb.taql(taqlcmd)
    chans = chans_tb.getcol('CHAN_FREQ')
    tb.close()
    
    ms.open(visil)
    ### Get the visibility statistics
    time_stat = np.zeros((len(scans),4))
    time_stat[:,0] = scans
    for i in range(0, len(scans)):
        # First get the time statistics
        spwkey = ''
        try:
            temp_time = ms.statistics(column='data',\
                                      complex_value = 'amp',\
                                     scan=str(scans[i]))
            time_stat[i][1] = temp_time[spwkey]['mean']
            time_stat[i][2] = temp_time[spwkey]['stddev']
            time_stat[i][3] = temp_time[spwkey]['max']
        except:
            print('No Data Here!')

    chan_stat = np.zeros((len(chans)/16,4))
    
    for i in range(0, len(chan_stat)):
        # First get the channel statistics
        spwa='0:'+str(i*16)+'~'+str(((i+1)*16)-1)
        spwkey = ''
        chan_stat[i][0] = ((chans[i*16]+chans[(((i+1)*16)-1)])/2.)/(10**6)
        try:
            temp_chan = ms.statistics(column='data',\
                                      complex_value = 'amp',\
                                      spw=spwa)
            chan_stat[i][1] = temp_chan[spwkey]['mean']
            chan_stat[i][2] = temp_chan[spwkey]['stddev']
            chan_stat[i][3] = temp_chan[spwkey]['max']
        except:
            print('No Data here!')
   
    ms.close()
    return [time_stat, chan_stat]


def uvspec_v2(visil):
    """retrieve the xaxis as well as amplitude and phase for uvspectrum"""

    # Do TAQL queries to get the necessary data
    tb.open(visil)
    taqlcmd = 'select DATA, FLAG from '+visil
    data_flag_tb = tb.taql(taqlcmd)
    data = data_flag_tb.getcol('DATA')
    flag = data_flag_tb.getcol('FLAG')

    taqlcmd = 'select CHAN_FREQ from '+visil+'/SPECTRAL_WINDOW'
    chans_tb = tb.taql(taqlcmd)
    chans = chans_tb.getcol('CHAN_FREQ')/(1e9)
    tb.close()

    # get unflagged data
    data *= (flag-1)*-1
    data[data==0]=complex(np.nan,np.nan)
    base_time_avg = np.nanmean(data,axis=2)

    return [chans, np.absolute(base_time_avg), np.angle(base_time_avg,deg=True)]

def time_convert(mytime, myunit='h'):
    """A task provided by CASA for time conversions."""
...
def amp_phase_time_v2(visil, specw):
    """Create the axis amplitude and phase plots."""

    # Do TAQL queries to get the necessary data
    tb.open(visil)
    taqlcmd = 'select DATA, FLAG, TIME from '+visil+' where DATA_DESC_ID == '+specw
    data_flag_tb = tb.taql(taqlcmd)
    data = data_flag_tb.getcol('DATA')
    flag = data_flag_tb.getcol('FLAG')
    time = data_flag_tb.getcol('TIME')
    tb.close()
    ...
    # put the time in relative units
    time_ax_raw = time.squeeze()
    time_ax = (time_ax_raw-time_ax_raw[0])/(3600)

    return [time_ax,np.absolute(data),np.angle(data,deg=True),time_convert(time_ax_raw[0],'s')]


def mask_check_plot_v2(visil):
    """Return the plotting axis for the uvspectrum of a source."""

    # Do TAQL queries to get the necessary data
    tb.open(visil)
    taqlcmd = 'select DATA, FLAG from '+visil+' where sqrt(sumsqr(UVW[:2])) < \
1600 && sqrt(sumsqr(UVW[:2])) > 1500 '
    data_flag_tb = tb.taql(taqlcmd)
    data = data_flag_tb.getcol('DATA')
    flag = data_flag_tb.getcol('FLAG')
    ...
    # get flagged and unflagged data
    uflag = data * ((flag-1)*-1)
    uflag[uflag==0]=complex(np.nan,np.nan)
    flagd  = data * flag
    flagd[flagd==0]=complex(np.nan,np.nan)

    amps=[np.absolute(uflag),np.absolute(flagd)] # calculates the amplitude

    # fix the xaxis
    freq=chans.squeeze()/(10**6)
    xaxis1 = np.array([freq,]*len(amps[0][0][0]))
    xaxis = np.swapaxes(xaxis1,0,1)
